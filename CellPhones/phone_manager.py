...
class PhoneAssignments():
    ...
    def add_employee(self, employee):
        ...
        for e in self.employees:
            if e.id == employee.id:
                raise PhoneError("Duplicate Employee ID #{} Unable to add")
    ...
    def add_phone(self, phone):
        ...
        for p in self.phones:
            if p.id == phone.id:
                raise PhoneError("Duplicate Phone ID #{} Unable to add")
    ...
    def assign(self, phone_id, employee):
        ...
        for p in self.phones:
            if p.id == phone_id:
                if p.is_assigned():
                    if employee.id == p.employee_id:
                        return
                    else:
                        raise PhoneError('The Phone is already assigned')
                else:
                    for ph in self.phones:
                        if employee.id == ph.employee_id:
                            raise PhoneError('Employee already has a phone')
                    p.assign(employee.id)
